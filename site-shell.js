(function(){
  ...
  async function openPage(key,push=true){
    if(loading||!PAGES[key])return; loading=true;
    try{
      const res=await fetch(PAGES[key].file+'?spa='+Date.now());
      const html=await res.text();
      const doc=new DOMParser().parseFromString(html,'text/html');
      const newMain=doc.querySelector('main');
      if(!newMain){location.href=PAGES[key].file;return}
  ...
  function bindNav(){
    document.querySelectorAll('a[data-page]').forEach(a=>{a.onclick=e=>{e.preventDefault();openPage(a.dataset.page,true)}});
    document.querySelectorAll('a[data-branch]').forEach(a=>{a.onclick=e=>{e.preventDefault();const cfg=PAGES[currentKey()]||PAGES.home;const b=cfg.branches[Number(a.dataset.branch)];let target=String(b[0]).startsWith('#')?document.querySelector(b[0]):document.querySelector(b[0]);if(target){target.scrollIntoView({behavior:'smooth',block:'start'})}}});
  }
  window.addEventListener('popstate',()=>{const k=currentKey();openPage(k,false)});
  window.__sageEditLoaded=!!document.querySelector('script[src*="edit-mode.js"]');
  window.__sageTasksLoaded=!!document.querySelector('script[src*="tasks.js"]');
