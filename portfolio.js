...
(function () {
  ...
  function renderProjects() {
    ...
    container.innerHTML = list.length
      ...
      : '<div class="empty">还没有项目，在下方新增一个吧。</div>';
  }
    ...
    return `
      <div class="card" data-id="${p.id}">
        ...
        <div class="task-actions" style="margin-top:8px">
          <button class="mini danger" onclick="portfolioDelete('${p.id}')">删除</button>
        </div>
      </div>`;
  }
  ...
  window.portfolioDelete = function (id) {
    ...
    save(load().filter(p => p.id !== id)); refresh();
  };

  function refresh() { renderProjects(); }
  ...
  window.__sagePortfolioRefresh = refresh;
})();
